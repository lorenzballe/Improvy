        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // NOTIFICATIONS — local reminders (no server). The toggles are
              // always shown (also in the web preview, where scheduling is a
              // no-op) so the UI can be reviewed anywhere.
                ...
                child: Column(
                  ...
                  children: [
                    _NotifToggleCard(
                      ...
                      tag: 'PRACTICE NUDGE',
                      description:
                          'One reminder a day at the time you choose — it skips '
                          'days you already practised, and points at your weakest '
                          'degree pair when it spots one.',
                      icon: Icons.notifications_active_rounded,
                      color: const Color(0xFFF59E0B),
                      value: provider.notifDaily,
                      onTap: () => provider.setNotifDaily(!provider.notifDaily),
                    ),
                    if (provider.notifDaily) ...[
                      const SizedBox(height: 12),
                      GestureDetector(
                        behavior: HitTestBehavior.opaque,
                        onTap: () async {
                          ...
                          if (picked != null) provider.setNotifTime(picked.hour, picked.minute);
                        },
                        child: Container(
                          ...
                          child: Row(
                            children: [
                              Icon(Icons.schedule_rounded, color: Colors.white.withAlpha(102), size: 18),
                              const SizedBox(width: 12),
                              Text('Reminder time',
                              ...
                              Container(
                                ...
                                child: Text(
                                  '${provider.notifHour.toString().padLeft(2, '0')}:${provider.notifMinute.toString().padLeft(2, '0')}',
                                  style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w900,
                                      color: Color(0xFFFBBF24), letterSpacing: 0.5),
                                ),
                              ),
                            ],
                          ),
                    ...
                    const SizedBox(height: 12),
                    _NotifToggleCard(
                      title: 'Comeback Reminders',
                      tag: 'AFTER 3+ DAYS AWAY',
                      description:
                          'A gentle nudge if you have been away for 3 days, one '
                          'more after a week — then we stay quiet.',
                      icon: Icons.waving_hand_rounded,
                      color: const Color(0xFF14B8A6),
                      value: provider.notifComeback,
                      onTap: () => provider.setNotifComeback(!provider.notifComeback),
                    ),
                  ],
                ),
              ),
...
// Toggle card for the NOTIFICATIONS section — same visual language as the
// Adaptive Difficulty card (tinted gradient + glow when ON), parametrised.
        ...
        decoration: BoxDecoration(
          gradient: value
              ? LinearGradient(
                  ...
                  colors: [color.withAlpha(77), color.withAlpha(20)],
                )
              : null,
          color: value ? null : Colors.white.withAlpha(8),
          border: Border.all(color: value ? color.withAlpha(102) : Colors.white.withAlpha(13)),
          borderRadius: BorderRadius.circular(24),
          boxShadow: value ? [BoxShadow(color: color.withAlpha(38), blurRadius: 30)] : null,
        ),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: value ? color : Colors.white.withAlpha(26),
                    borderRadius: BorderRadius.circular(16),
                    boxShadow: value ? [BoxShadow(color: color.withAlpha(102), blurRadius: 25)] : null,
                  ),
                  child: Icon(icon, color: value ? Colors.white : Colors.white.withAlpha(102), size: 24),
                ),
                const SizedBox(width: 16),
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(title,
                          style: TextStyle(fontSize: 14, fontWeight: FontWeight.w900,
                              color: value ? Colors.white : Colors.white.withAlpha(179),
                              letterSpacing: 0.4)),
                      const SizedBox(height: 3),
                      Text(tag,
                ...
                _ToggleSwitch(value: value, color: color),
              ],
            ),
            const SizedBox(height: 16),
            Text(description,
                style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500,
                    color: Colors.white.withAlpha(102), height: 18 / 11)),
          ],
        ),
      ),
    );
